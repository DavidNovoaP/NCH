...
def IF_train (data, n_estimators, contamination, random_state):
    print("IF trainning...")
    model = IsolationForest(n_estimators=n_estimators, behaviour="new", contamination=contamination, random_state=random_state)
    model.fit(data)
    print("IF trained.")
    return model
...
def AE_train (data, hidden, epochs, sensivity_threshold):
    print("AE trainning...")
    h2o.init(port = 54321)
    ...
    print("AE trained.")
    return model, margenExtremadamenteAtipico
